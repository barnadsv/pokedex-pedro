<script lang="ts">
    ...
    import { clamp, round, adjust } from '$lib/utils/math';
    ...
    let thisCard: HTMLDivElement | undefined;
    let repositionTimer: string | number | NodeJS.Timeout | undefined;
    ...
    let showcaseInterval: string | number | NodeJS.Timeout | undefined;
    let showcaseTimerStart: string | number | NodeJS.Timeout | undefined;
    let showcaseTimerEnd: string | number | NodeJS.Timeout | undefined;
    ...
    const interact = (e: { type: string; clientX: number; touches: { clientY: any; }[]; clientY: number; target: any; }) => {
    ...
    const interactEnd = (e: null | undefined, delay = 500) => {
      setTimeout(function () {
    ...
    const orientate = (/** @type {{ (e: any): { absolute: any; relative: { alpha: number; 
		// data / pokemon props
		beta: number; gamma: number; }; }; relative?: any; }} */ e) => {
    ...
    const updateSprings = ( /** @type {{ x: number; y: number; }} */ background, /** @type {{ x: number; y: number; }} */ rotate, /** @type {{ x: number; y: number; o: number; }} */ glare ) => {
    ...
    $: {
      if ($activeCard && $activeCard === thisCard && browser) {
        interacting = true;
